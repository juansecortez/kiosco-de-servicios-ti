...
function zonasUbicacionReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case 'DELETE_ZONA_FAILURE':
      return { ...state, error: action.payload };
    case 'FETCH_UBICACIONES_BY_ZONA_SUCCESS':
      return { ...state, ubicaciones: action.payload };
    case 'FETCH_UBICACIONES_BY_ZONA_FAILURE':
      return { ...state, error: action.payload };
    ...
    case 'EDIT_UBICACION_SUCCESS':
      return {
        ...state,
        ubicaciones: state.ubicaciones.map((ubicacion) =>
          ubicacion.ID === action.ubicacion.idpayload.id ? action.payload : ubicacion,
        ),
      };
    ...
    case 'DELETE_UBICACION_SUCCESS':
      return {
        ...state,
        ubicaciones: state.ubicaciones.filter((ubicacion) => ubicacion.ID !== action.payload.id),
      };
